...
';
    ...
    if (is_user_logged_in()) {
        ...
        echo '<div> <form method="post">
                <input class="mb-5" type="submit" name="btn_assets" value="Click here to get your MediaVerse assets"></form>
              </div>';
...
function get_asset($token)
{

    $url = "https://dashboard.mediaverse.atc.gr/dam/assets/";
    $curl = curl_init($url);
...
function save_assets($token)
{
    ...
    $key_name = get_asset($token);

    if ($key_name) {

        $results = array();
//    foreach (array_combine($key_name[0], $key_name[1]) as $key => $name)
        for ($i = 0, $count = count($key_name[0]); $i < $count; $i++) {
            $key = $key_name[0][$i];
            $name = $key_name[1][$i];
...
//        print_r($result); // prints the contents of the collected file before writing..
            // upload directory
//        check the upload_path for automated upload
...
//            $output_filename_screenshot = $screenshot_key. '.png';
                ...
                //add this string before the image string, because function for uploading assets has this argument format
